...
#else
                        while (!get_sys_bit(*map, src_id)) src_id++; goto quit_search;
#endif
                }
        }
...
/* Try to receive an asynchronous message from 'src_ptr' */
  ...
  caller_ptr = src_ptr; /* Needed for A_ macros later on */ /* Scan the table */
  do_notify = FALSE;
  done = TRUE;
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {
        ...
        if(q==USER_Q)
        {
                ...
                while(find!=NULL)
                {
                     ...
                     {
                       if(schedproc[i].endpoint == find-> p_endpoint)
                       {
                        if(minfss > schedproc[i].fss_priority)
                        {
rp = find;
                                minfss = schedproc[i].fss_priority;
                        }

                        find = find->p_nextready;
                        break;
                      }
                    }
                }
